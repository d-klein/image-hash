...
def rad(n,m,rho):
    ...
    for s in xrange(0,upto+1):
        nom = misc.factorial(n-s)
        denom1 = misc.factorial(((n+abs(m)) / 2) - s) * misc.factorial(s)
        denom2 = misc.factorial(((n-abs(m)) / 2) - s)
        frac = nom / (denom1 * denom2)
        res = ((-1) ** s) * (rho ** (n-2*s)) * frac
        sum_ = sum_ + res
    return sum_
...
# TODO AVOID PRINTING REDUNDANT MOMENTS
